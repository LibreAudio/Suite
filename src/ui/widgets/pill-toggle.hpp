...
public:
        ...
        : LibreAudioContainerSubWidget(parent)
    {
        ...
        uint cellWidth = 0;
        fCells.reserve(ARRAY_SIZE(test));

        for (uint i = 0; i < ARRAY_SIZE(test); ++i)
        {
            std::unique_ptr<LibreAudioBackgroundPillToggleCellWidget> widget { new LibreAudioBackgroundPillToggleCellWidget(this, this, test[i]) };
