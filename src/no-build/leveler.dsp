...
maxboost = vslider("h:[2]Controls/[2][unit:dB]max boost", init_leveler_maxboost, 0, 30, 1);
max_cut = vslider("h:[2]Controls/[3][unit:dB]max cut", init_leveler_maxcut, 0, 30, 1) : ma.neg;
scale_boost = vslider("h:[2]Controls/[4][unit:%]boost scale", init_leveler_boost_scale, 0, 100, 1) * 0.01;
scale_cut = vslider("h:[2]Controls/[5][unit:%]cut scale", init_leveler_cut_scale, 0, 100, 1) * 0.01;

leveler_speed_up = vslider("h:[2]Controls/[6][unit:%]speed up", init_leveler_speed_up, 0, 100, 1) * 0.01;
leveler_speed_down = vslider("h:[2]Controls/[7][unit:%]speed down", init_leveler_speed_down, 0, 100, 1) * 0.01;
leveler_brake_thresh = target + vslider("h:[2]Controls/[8][unit:dB]brake threshold", init_leveler_brake_threshold,-90,0,1)+32;
meter_leveler_brake = _*100 : vbargraph("h:[2]Controls/[9][unit:%]brake",0,100);

leveler_meter_gain = vbargraph("h:[2]Controls/[10][unit:dB]gain",-50,50);

postGainSlider = vslider("h:[2]Controls/[11][unit:dB]PostGain", 0, -20, 20, 0.1);

meter_lufs_in = vbargraph("h:[2]Controls/[unit:dB]lufs in", meters_minimum, 0);
meter_lufs_out = vbargraph("h:[2]Controls/[unit:dB]lufs out", meters_minimum, 0);

vad = vad_ext : vad_smooth;
vad_ext = vslider("h:[2]Controls/[12]vad_ext[symbol:vad_ext]", 0, 0, 1, 0.001); // from external voice activity detection (0 = no voice, 1 = voice)
vad_smooth = si.smoo;

detection_mode = vslider("h:[2]Controls/[13]detect[symbol:detection_mode]", 0, 0, 1, 0.01); // 0 = internal expander only, 1 = external vad only, 0.5 = blend of both

brake_hp_freq = vslider("h:[2]Controls/[14][unit:Hz][scale:log]brake hp", init_leveler_brake_hp, 1, 500, 1);
brake_lp_freq = vslider("h:[2]Controls/[15][unit:Hz][scale:log]brake lp", init_leveler_brake_lp, 20, 20000, 1);
